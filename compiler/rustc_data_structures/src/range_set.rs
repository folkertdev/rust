    ...
    T: core::ops::Sub<Output = T>,
{
    ...
    /// The ranges present in both `self` and `other`.
    ...
    pub fn difference(&self, other: &Self) -> Self {
        ...
        for &(a_offset, a_size) in a.0.iter() {
            ...
            // Skip ranges of `b` that end before this range of `a` begins. Since both sequences are
            // sorted they cannot overlap any later range of `a` either.
            ...
            {
                if b_offset > cursor {
                    out.push((cursor, b_offset - cursor));
                }
                cursor = Ord::max(cursor, b_offset + b_size);
                k += 1;
            }
            // Keep the remainder of `a`'s range.
            if cursor < a_end {
                out.push((cursor, a_end - cursor));
            }
        }
        Self(out)
    }
